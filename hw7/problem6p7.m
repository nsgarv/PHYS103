...
L = 1; %input('Enter system length: ');  
nstep = 500; %input('Enter number of time steps: ');
% The system extends from x=-L/2 to x=L/2
kappa = 1.;
h = L/(N);  % Grid size
x = ((1:N)-1/2)*h - L/2; % coordinate point of grid
length(x)
coeff = -kappa*tau/(2.*h);
coeff2 = kappa*tau;
tt = linspace(1e-4,.03, nstep);
length(tt)
sigma = sqrt(2*kappa.*tt); 
if( coeff < 0.5 )
...
nn(round(N/2)) = (ones(length(x),1)*(1./sigma)).*exp(-(x'-L/4).^2*(1./sigma.^2)/2)/sqrt(2*pi);   % Initial cond. is delta function in center
